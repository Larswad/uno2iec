	...
	do {
		...
		if('L' == resp) { // Host system will give us something else if we're at last line to send.
			...
			if(len == actual) {
				...
				sprintf_P(serCmdIOBuf, "Expected: %d chars, got %d.", len, actual);
				Log(Error, FAC_IFACE, serCmdIOBuf);
			}
		}
		else {
			if('l' not_eq resp) {
				sprintf_P(serCmdIOBuf, "Ending at char: %d.", resp);
				Log(Error, FAC_IFACE, serCmdIOBuf);
...
void Interface::sendFile()
{
	...
	do {
		...
		if(2 not_eq len) {
			strcpy_P(serCmdIOBuf, "2 Host bytes expected, stopping");
			Log(Error, FAC_IFACE, serCmdIOBuf);
			success = false;
			break;
		}
		...
		if('B' == resp or 'E' == resp) {
			...
			if(actual not_eq len) {
				strcpy_P(serCmdIOBuf, "Host bytes expected, stopping");
				success = false;
				Log(Error, FAC_IFACE, serCmdIOBuf);
				break;
			}
			...
			strcpy_P(serCmdIOBuf, "Got unexp. cmd resp.char.");
			Log(Error, FAC_IFACE, serCmdIOBuf);
			success = false;
		}
	...
	if(success) {
		sprintf_P(serCmdIOBuf, "Transferred %u of %u bytes.", bytesDone, totalSize);
...
byte Interface::handler(void)
{
	...
	if(retATN == IEC::ATN_ERROR) {
		strcpy_P(serCmdIOBuf, "ATNCMD: IEC_ERROR!");
		Log(Error, FAC_IFACE, serCmdIOBuf);
		reset();
	}
...
#ifdef CONSOLE_DEBUG
		{
			sprintf_P(serCmdIOBuf, "ATN code:%d cmd: %s (len: %d) retATN: %d\r\n", m_cmd.code, m_cmd.str, m_cmd.strLen, retATN);
...
char* Interface::dateTimeString(char* dest, bool timeOnly)
{
	...
	// format the string(s).
	if(not timeOnly)
		sprintf_P(dest, "%04u02u02u ", m_year, (word)m_month, (word)m_day);
	else
		dest[0] = '\0';
	byte timePos = strlen(dest);
	sprintf_P(&dest[timePos], "%02u02u02u", (word)m_hour, (word)m_minute, (word)m_second);
...
/*			serCmdIOBuf[len] = '\0';
			if('n' == resp) {
			sprintf_P(serCmdIOBuf, "SAVED, Exp: %d chars, got %d.", len, actual);
			} else {
			sprintf_P(serCmdIOBuf, "LOADED, Exp: %d chars, got %d.", len, actual);
			}
...
*/
			...
			sprintf_P(serCmdIOBuf, "Exp: %d chars, got %d.", len, actual);
			Log(Error, FAC_IFACE, serCmdIOBuf);
		}
	}
	else if('C' == resp) {
