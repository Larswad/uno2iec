...
IEC::IEC(uchar deviceNumber)
  : m_state(noFlags), m_deviceNumber(deviceNumber),
    m_atnPin(3), m_dataPin(4), m_clockPin(5)
{
...
uchar IEC::timeoutWait(uchar waitBit, bool whileHigh)
{
  ushort t = 0;
  bool c;
  ...
  while(t < TIMEOUT) {
    ...
    us_spin(2); // The aim is to make the loop at least 3 us
    t++;
  }
...
uchar IEC::receiveByte(void)
{
  ...
  while(readCLOCK() and (n < 20)) {
    us_spin(10);  // this loop should cycle in about 10 us...
    n++;
  }
...
//
bool IEC::sendByte(uchar data, bool signalEOI)
{
...
uchar IEC::deviceNumber() const
{
  return m_deviceNumber;
} // deviceNumber


void IEC::setDeviceNumber(const uchar deviceNumber)
{
